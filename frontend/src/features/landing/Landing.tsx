...
const CHIPS = [
  { label: "Detect", icon: <IconRadar />, gradient: "bg-gradient-to-br from-navy-950 to-navy", rotate: -8, style: { left: "2%", top: "4%" } },
  { label: "Score", icon: <IconGauge />, gradient: "bg-gradient-to-br from-brand-dark to-brand", rotate: 6, style: { left: "15%", top: "20%" } },
  { label: "Recommend", icon: <IconSpark />, gradient: "bg-gradient-to-br from-gold-dark to-gold", rotate: 8, style: { right: "2%", top: "4%" } },
  { label: "Diagnose", icon: <IconBrain />, gradient: "bg-gradient-to-br from-ink-800 to-ink", rotate: -5, style: { right: "15%", top: "20%" } },
  { label: "Execute", icon: <IconBolt />, gradient: "bg-gradient-to-br from-navy to-brand-dark", rotate: 7, style: { left: "2%", top: "66%" } },
  { label: "Audit", icon: <IconLedger />, gradient: "bg-gradient-to-br from-navy-950 to-ink", rotate: -6, style: { left: "15%", top: "80%" } },
  { label: "Guardrail", icon: <IconShield />, gradient: "bg-gradient-to-br from-gold to-gold-light", rotate: -7, style: { right: "2%", top: "66%" } },
  { label: "Measure", icon: <IconChart />, gradient: "bg-gradient-to-br from-ink-800 to-navy-950", rotate: 6, style: { right: "15%", top: "80%" } },
] as const;
  ...
  return (
    <div>
      ...
      <section className="relative mx-auto max-w-6xl overflow-hidden rounded-3xl bg-ink px-6 py-24 text-center sm:px-12">
        ...
        <div className="relative mx-auto max-w-lg lg:max-w-xl">
          ...
          <h1 className="mt-4 font-display text-5xl font-bold tracking-tight text-white sm:text-6xl">
            Recover<span className="text-gold">.</span>
            <br />
            Never unsupervised<span className="text-gold">.</span>
          </h1>
          ...
          <div className="mt-8 flex flex-wrap items-center justify-center gap-3">
            <button
              ...
              className="rounded-full bg-white px-6 py-3 text-sm font-semibold text-ink transition hover:bg-gold hover:text-ink"
            >
              Open the console
            </button>
            <button
              onClick={onHowItWorks}
              className="flex items-center gap-1.5 rounded-full border border-white/20 px-6 py-3 text-sm font-semibold text-white transition hover:border-gold hover:text-gold"
            >
      ...
      <section className="mx-auto grid max-w-6xl grid-cols-1 items-center gap-10 px-6 py-10 sm:px-12 lg:grid-cols-2 lg:gap-16">
        ...
        <div>
          <div className="text-xs font-semibold uppercase tracking-[0.2em] text-gold-dark">Not a demo script</div>
          ...
          <button
            onClick={onHowItWorks}
            className="mt-6 inline-flex items-center gap-1.5 rounded-full bg-gold px-5 py-2.5 text-sm font-bold text-ink transition hover:bg-gold-dark"
          >
      ...
      <section className="mx-auto max-w-6xl px-6 py-16 sm:px-12">
        <div className="grid grid-cols-2 gap-4 lg:grid-cols-4">
          ...
          <StatBadge value="0" label="Unauthorized actions — by design" gold />
        </div>
      </section>
      ...
      <section className="relative mx-auto max-w-6xl overflow-hidden rounded-3xl bg-ink px-6 py-16 text-center sm:px-12">
        ...
        <div className="relative">
          <h2 className="font-display text-4xl font-bold text-white sm:text-5xl">
            See the <span className="text-gold">process</span>, not just the result.
          </h2>
          ...
          <button
            onClick={onOpenConsole}
            className="mt-8 rounded-full bg-white px-6 py-3 text-sm font-semibold text-ink transition hover:bg-gold"
          >
            Open the console
          </button>
        </div>
...
function StatBadge({ value, label, gold }: { value: string; label: string; gold?: boolean }) {
  return (
    <div className="rounded-2xl border border-ink/10 bg-white p-6 text-center shadow-card">
      <div className={`font-display text-4xl font-bold ${gold ? "text-gold-dark" : "text-ink"}`}>{value}</div>
